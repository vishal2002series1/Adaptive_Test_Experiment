...
if st.session_state.phase == 'setup':
    ...
    with col1:
        student_id = st.text_input("Student ID", value="Data Science Student V2")
    with col2:
        target_exam = st.selectbox("Target Exam", ["UPSC", "AWS SOLUTIONS ARCHITECT", "SSC CGL", "JEE MAINS"])
    ...
    with col3:
        target_subject = st.text_input("Target Subject", value="ALL SYLLABUS")
        target_topic = st.text_input("Target Topic", value="ALL SYLLABUS")
    with col4:
...
# ==========================================
    ...
    st.write("Please answer the following questions. (LaTeX will render automatically)")
    st.divider()
    
    # Render Questions
    for i, q in enumerate(st.session_state.questions):
        # st.markdown natively renders LaTeX wrapped in $ or $$
        st.markdown(f"**Q{i+1}.** {q['text']}")
        
        # Format options for the radio button
        options_list = [f"{key}: {val}" for key, val in q['options'].items()]
        ...
        # Extract just the letter (A, B, C, D) if they made a choice
        if choice:
...
# ==========================================
    ...
    for result in eval_data.get("graded_results", []):
        ...
        with st.expander(f"Question: {original_q['text'][:60]}..."):
            st.markdown(f"**Full Question:** {original_q['text']}")
            ...
            st.markdown(f"**Explanation:** {original_q['explanation']}")
